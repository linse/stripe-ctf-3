...
func (s *Server) sqlHandler(w http.ResponseWriter, req *http.Request) {
  ...
  var resp []byte
    ...
    for {
      req, err := s.client.SafePost(s.cluster.primary.ConnectionString, "/sql", req.Body)
      myresp, _ := ioutil.ReadAll(req)
      resp = myresp
      if err != nil {
        log.Printf("Couldn't forward query to %v: %s", s.cluster.primary, err)
        time.Sleep(5000)
      } else {
        break
      }
