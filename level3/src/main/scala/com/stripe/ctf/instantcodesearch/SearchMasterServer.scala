...
class SearchMasterServer(port: Int, id: Int) extends AbstractSearchServer(port, id) {
  ...
  override def query(q: String) = {
    ...
    // dont just take the first of the sequence, but the first Future that finishes
    Future.select(responses).map ({case (a:Try[HttpResponse], b:Seq[Future[HttpResponse]])=>a.getOrElse(errorResponse(HttpResponseStatus.BAD_GATEWAY, "No query ever returned"))})
  }
}
